...
<body>
    ...
    <div class="container mt-4">
        ...
        <div class="mb-3">
            <form action="/kbgm/member/list_member.php" method="GET" class="d-flex">
                <input class="form-control me-2" type="search" name="keyword" value="<?= htmlspecialchars($keyword) ?>" placeholder="Cari No. KBGM / Nama" aria-label="Search">
                <button class="btn btn-outline-primary" type="submit">Cari</button>
                <?php if (!empty($keyword)): ?>
                    <a href="/kbgm/member/list_member.php" class="btn btn-outline-secondary ms-2">Reset</a>
                <?php endif; ?>
            </form>
        </div>
            ...
            <tbody>
                ...
                if (!empty($members_data)) {
                    foreach ($members_data as $row) {
                        ...
                        if ($row['status'] == 1) {
                            // Panggil API untuk mendapatkan no_rm yang dicocokkan
                            ...
                            $status_label = '<a href="/kbgm/includes/matching.php?no_kbgm=' . $no_kbgm . '" class="btn btn-warning btn-sm">Cocokkan</a>';
                        }
                        ...
                        echo "<td>
                                <a href='edit_member.php?no_kbgm=" . htmlspecialchars($no_kbgm) . "' class='btn btn-sm btn-primary'>
                                    Edit
                                </a>
                                <a href='../visits/list_visits.php?no_kbgm=" . htmlspecialchars($no_kbgm) . "' class='btn btn-sm btn-primary'>
                                    Lihat Kunjungan
                                </a>
                                <button class='btn btn-sm btn-info' onclick='cetakMember(this,
                    ...
                    echo "<tr><td colspan='8' class='text-center'>Data member tidak ditemukan.</td></tr>";
                }
                ?>
            </tbody>
        </table>
